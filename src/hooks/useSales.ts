...
export const useCustomerBalanceQuery = (customerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: () =>
      customerId
        ? saleService.getCustomerBalance(customerId)
        : { total_sales: 0, total_payments: 0, outstanding_credit: 0 },
    enabled: !!customerId,
    staleTime: 1000 * 60 * 2, // 2 minutes
  });
};
...
export const useCustomerLedgerQuery = (customerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: () =>
      customerId ? saleService.getCustomerLedger(customerId) : { sales: [], payments: [] },
    enabled: !!customerId,
    staleTime: 1000 * 60 * 2, // 2 minutes
  });
};
