...
import type { SaleWithCustomer } from '../services/sale.service';
...
export const useCreateSaleMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      // Invalidate sales feeds
      queryClient.invalidateQueries({ queryKey: ['sales', 'list', variables.shop_id] });
      // Invalidate specific customer balances & ledgers
      ...
      queryClient.invalidateQueries({ queryKey: ['dashboard', 'sales', variables.shop_id] });
    },
  });
};
...
export const useDeleteSaleMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['dashboard', 'sales', variables.shopId] });
    },
  });
};
