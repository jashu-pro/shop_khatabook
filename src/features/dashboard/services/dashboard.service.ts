...
export const dashboardService = {
  ...
  async getDashboardMetrics(ownerId: string): Promise<DashboardMetrics> {
    ...
    try {
      ...
      // Calculate totals
      const totalSalesSum = salesList.reduce((sum, s) => sum + Number(s.total_amount), 0);
      const totalPaymentsSum = paymentsList.reduce((sum, p) => sum + Number(p.payment_amount), 0);
      ...
      const todaySales = salesList
        .filter(s => new Date(s.sale_date) >= startOfToday)
